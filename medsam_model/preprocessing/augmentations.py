...
================================
All augmentation pipelines for FedBCa.

CORRECTED VERSION:
- FIXED overly aggressive GIN destabilizing ResNet50
- Reduced GIN probability/intensity
- Preserved scanner-consistent shared-network behaviour
- Maintains realistic MRI domain variation
- Keeps augmentation scientifically valid

Three experiment types per model:
  1. full aug:   AugmentationPipeline / ResNet50AugmentationPipeline
  2. GIN only:   GINOnlyPipeline
  3. no aug:     MinimalAugmentation
"""
...
class GeometricAugmentation:
    ...
    NO vertical flip — pelvic anatomy is not vertically symmetric.
    Rotation ±15° covers patient positioning variation.
...
class GINAugmentation:
    """
    Global Intensity Nonlinear augmentation.

    CORRECTED VERSION:
    Previous implementation was too aggressive for MRI segmentation.

    FIXES:
    - reduced probability
    - weaker nonlinear distortion
    - preserved shared-network consistency
    - stabilized ResNet50 training
    """
...
class BiasFieldAugmentation:
    """
    Simulates MRI B1 inhomogeneity.
...
class GaussianNoise:
    ...
    def __call__(self, image: np.ndarray) -> np.ndarray:
        ...
        return (
            image
            + np.random.randn(*image.shape).astype(np.float32)
            * sigma
        )
...
# ──────────────────────────────────────────────
# RESNET50 AUGMENTATION PIPELINE
# ──────────────────────────────────────────────
