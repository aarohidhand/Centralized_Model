...
utils/trainer.py
=================
Universal training engine for EfficientNet, ResNet50, Swin Hybrid, and MedSAM.

FINAL STABLE VERSION:
- Keeps current V3 behavior for Swin / EfficientNet / MedSAM
- Adds ONLY ResNet-specific LR tracking stabilization
- Does NOT affect other model experiments
"""
...
def build_optimizer(model, config):
    ...
    lr_enc = (
        lr_base *
        config.get('enc_lr_ratio', 0.10)
    )

    wd = config.get('weight_decay', 1e-4)

    # MedSAM image encoder
    medsam_keywords = {'image_encoder'}

    # Swin encoder
    lr_swin = (
        lr_base *
        config.get('swin_lr_ratio', 0.30)
    )
    ...
    for name, param in model.named_parameters():
        ...
        if any(k in name for k in medsam_keywords):
            medsam_params.append(param)

        elif any(k in name for k in swin_keywords):
...
def train_model(
    ...
    warmup_steps = (
        config.get('warmup_epochs', 10)
        * steps_per_ep //
        accum_steps
    )

    min_lr_ratio = config.get(
        'min_lr_ratio',
        0.05
    )
    ...
    cls_rampup = config.get(
        'cls_rampup_epochs',
        10
    )

    target_cls_w = config.get(
        'cls_weight',
        0.15
    )
    ...
    print(
        f'  Eff batch : '
        f'{config.get("batch_size",4) * accum_steps}'
    )

    print(
        f'  Rampup    : '
        f'cls gradient starts ep {cls_rampup}, '
        f'full at ep {cls_rampup * 2}, '
        f'combined scoring from ep {rampup_done}'
    )
    ...
    for epoch in range(epochs):
        ...
        # ─────────────────────────────────────
        # RESNET-SPECIFIC LR TRACKING FIX
        # ONLY affects ResNet logging behavior
